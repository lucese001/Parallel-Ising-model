...
inline void print_performance_summary(double total, double compute,
    ...
    master_printf("Computation time:          %10.3f s (%5.1f%%)\n", compute, 100.0*compute/total);
...
inline void print_simulation_info(int N_dim, long long N, size_t nThreads, int nConfs,
                                   double Beta, int world_size) {
    master_printf("N_dim: %d, Npunti: %zu, NThreads: %d, nConfs: %d, Beta: %lg\nnumero di rank: %d\n",
                  N_dim, N, nThreads, nConfs, Beta, world_size);
}
    ...
    if (world_rank == 0) {
        ...
        if (N_dim == 2) {
            ...
            for (size_t i = 0; i < N_global; ++i) {
                master_printf("%+d ", (int)global_conf[i]);
                if ((i + 1) % 16 == 0 && i + 1 < N_global) master_printf("\n  ");
            }
            master_printf("\n");
        }
        master_printf("==============================\n");
